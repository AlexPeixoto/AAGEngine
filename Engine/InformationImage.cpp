...
InformationImage::InformationImage(TextControl* textControl, const std::string& path, const std::string& text, Point2i imagePosition, Point2i textPosition, Vector2f imageScale, Vector2f textScale, Color textColor) : textControl(textControl), text(text), imagePosition(imagePosition), textPosition(textPosition), imageScale(imageScale), textScale(textScale), textColor(textColor) {
...
void InformationImage::setTextColor(const Color& textColor) {
	this->textColor = textColor;
}
std::string InformationImage::getImagePath() const {
	return image->getPath();
}
Image* InformationImage::getImage() const {
	return image;
}
std::string InformationImage::getText() const {
	return text;
}
