...
Sprite::Sprite(const std::string& path, int tileWidth, int tileHeight) : Image(path) {
...
Sprite::Sprite(const std::string& path, Vector2i tileSize) : Image(path) {
...
void Sprite::next() {
	if (textureSize.x / ((tileSize.x*(index.x + 1))) >= 1) {
		--index.x;
	}
	else if (loopToBegin) {
		index.x = loopTo;
...
void Sprite::nextRow() {
	if (textureSize.y / ((textureSize.y*(index.y + 1))) >= 1) {
		++index.y;
	}
}
void Sprite::prevRow() {
...
void Sprite::setColumn(int column) {
	if (textureSize.x / ((float)(textureSize.x*(column + 1))) >= 0 && column > 0) {
		index.x = column;
	}
}
