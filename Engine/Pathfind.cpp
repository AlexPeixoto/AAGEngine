...
int Pathfind::analyzeNode(int* map, Point2i origin, Vector2i size) {
	...
	int clearance = 1;
	//This represents on which axis I can expand the most.
	int minSize = size.x < size.y ? size.x : size.y;
	//CHECK THIS, IT LOOKS WRONG
	for (; clearance < minSize; ++clearance) {
		//Min size, this cannot be true
		if (position.x + clearance >= size.x || position.y + clearance >= size.y) {
			return clearance;
		}
		/*
			for (int movingY = position.y; movingY >= 0; --movingY){
						//This represents Y //This represents X (moving)
				if (map[((position.y - movingY)*size.x) + (position.x + clearance)] < 0) {
					return clearance;
				}
				         //This is the Y moving             //This represents the X
				if (map[((position.y + clearance + pos)*size.x) + position.x] < 0) {
					return clearance;
				}
			}

			for (int movingX = position.x; movingX >= 0; --movingX){
				//This is the Y moving             //This represents the X
				if (map[((position.y + clearance)*size.x) + position.x - movingX ] < 0) {
					return clearance;
				}
			}
		*/
		if (map[(position.y*size.x) + position.x + clearance] < 0) {
			return clearance;
		}
		if (map[((position.y + clearance)*size.x) + position.x] < 0) {
			return clearance;
		}
	}
	return clearance;
...
void Pathfind::addNodeAndCheck(int* clearanceMap, int* movementMap, Vector2i size, Point2i origin, Node* node, std::vector<Node*>& ol, std::vector<Node*> cl, std::vector<int>& iol) {
	...
	if (isValid(size, Point2i(origin.x, origin.y))) {
		...
		tmpNode->setMovementCost(movementMap[origin.y*size.x + origin.x]);
...
int Pathfind::hMethod(Point2i origin, Point2i destiny) {
	if (!diagonal) {
		return (abs(origin.x - destiny.x) + abs(origin.y - destiny.y));
	}
	else {
		return static_cast<int>(sqrt((pow(abs(origin.x - destiny.x), 2) + pow(abs(origin.y - destiny.y), 2))));
	}
}
...
Node* Pathfind::generatePathfind(Point2i origin, Point2i destiny, int clearance) {
	...
	while (!ol.empty()) {
		...
		for (size_t x = 0; x < current->getNodes().size(); x++) {
