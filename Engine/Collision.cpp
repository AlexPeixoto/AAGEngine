...
Point2i Collision::penetrationAABB(BoundingBox b1, BoundingBox b2){
    ...
    if(!collidedAABB(b1, b2))return penetration;
    else{
        penetration.x = b1.position.x + b1.size.x - b2.position.x;
        penetration.y = b1.position.y + b1.size.y - b2.position.y;
    }
    return penetration;
}
...
void Collision::getSpriteAxis(Adventure::Sprite* sprite, Point2f* axis){
    ...
    axis[1] = (Point2f)trans.transformPoint(local.width, 0.f);
    axis[2] = (Point2f)trans.transformPoint(local.width, local.height);
    axis[3] = (Point2f)trans.transformPoint(0.f, local.height);
}
        ...
        if (((minOBB2<=maxOBB1)&&(maxOBB2>=minOBB1))){
            if(i==0||i==2){
                if(maxOBB1-minOBB2 > penetrationX)
                    penetrationX=maxOBB1-minOBB2;
            }
            if(i==1||i==3){
                if(maxOBB1-minOBB2 > penetrationY)
                    penetrationY=maxOBB1-minOBB2;
            }
        }
    }
