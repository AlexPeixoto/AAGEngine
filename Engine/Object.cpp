...
bool Object::loadSprite() {
	...
	objectSprite = new (std::nothrow)Sprite(path, tileSize);
	if (objectSprite) {
		return true;
	}
	return false;
...
void Object::addProperty(const std::string& key, const std::string& value) {
	...
	if (properties->find(key) != properties->end())
		throw std::runtime_error("[Object] Cannot add a duplicated key on the properties list");
...
std::string Object::getProperty(const std::string& key) const {
	if (properties->find(key) == properties->end()) {
		throw std::runtime_error("[Object] Cannot find the property with the specified key");
	}
...
void Object::addItem(int id, int quantity) {
	...
	if (items->find(id) != items->end()) {
		throw std::runtime_error("[Object] Cannot add a duplicated key on the items list");
	}
...
int Object::getItemQuantity(int id) const {
	if (items->find(id) == items->end()) {
		throw std::runtime_error("[Object] Cannot find the item with the specified id");
	}
	return items->at(id);
}
