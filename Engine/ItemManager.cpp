...
std::vector<Item*> *ItemManager::itemList = new std::vector<Item*>();
std::string ItemManager::relativePath = "";
...
Item* ItemManager::getItem(size_t id) {
	for (const auto& item : *itemList) {
		if (item->id == id) {
			return item;
		}
...
bool ItemManager::loadFromFile(const std::string& path) {
	std::string loadPath;
	...
	for (int x = 0; x < size; x++) {
		...
		for (int y = 0; y < pSize; y++) {
			...
			if (i->properties->find(std::string(kChar)) != i->properties->end()) {
			...
			i->properties->insert(std::make_pair(std::string(kChar), std::string(kValue)));
			free(kChar);
			free(kValue);
		}
	}
...
bool ItemManager::addItem(Item* item) {
	if (ItemManager::getItem(item->id)) {
		return false;
	}
